...
export const SideMenu = () => {
    return (
        <div className=" flex flex-grow flex-col gap-4 justify-left font-bold font-sans w-1/6 pl-10 pt-10 border-r ">
            <SideBarItem url={"/dashboard/home"} icon={<CiHome />} title="Home" />
            <SideBarItem url={"/dashboard/transfer"} icon={<GrTransaction />} title="Transfer" />
            <SideBarItem url={"/dashboard/transaction"} icon={<FaHistory />} title="Transactions" />
            <SideBarItem url={"/dashboard/p2p"} icon={<RiP2pFill />} title="P2P" />
        </div>
    )
}
